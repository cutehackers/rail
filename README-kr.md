...
2) publish script를 실행합니다.

```bash
./publish.sh v0.6.1
```

이 스크립트는 release metadata를 검증하고, `pyproject.toml`과 `uv.lock`을
갱신하고, `scripts/release_gate.sh`를 실행하고, 필요한 release metadata
변경분을 커밋한 뒤 `main`과 release tag를 push합니다.
