...
해당 버전의 `CHANGELOG.md` 최상단 항목이 없으면 스크립트는 이전 태그,
커밋 요약, 변경 파일, release contract, 최근 changelog 스타일을 바탕으로
agent용 changelog 작성 가이드를 출력하고 release metadata를 바꾸기 전에
중단합니다. agent가 `CHANGELOG.md`만 수정하게 한 뒤 diff를 확인하고 같은
명령을 다시 실행합니다.

changelog 항목이 있으면 스크립트는 changelog 품질을 검증하고,
`pyproject.toml`과 `uv.lock`을 갱신하고, `scripts/release_gate.sh`를
실행하고, 필요한 release metadata 변경분을 커밋한 뒤 `main`과 release
tag를 push합니다.
