...
def test_codex_vault_runtime_blocks_target_mutation_even_when_codex_exits_nonzero(tmp_path):
    target = _target_repo(tmp_path)
    (target / "app.txt").write_text("old\n", encoding="utf-8")
    handle = rail.start_task(_draft(target))
    runner = FakeCodexRunner(
...
def test_codex_vault_runtime_audits_policy_events_before_malformed_json(tmp_path):
    handle = rail.start_task(_draft(_target_repo(tmp_path)))
    runner = FakeCodexRunner(
        final_output={
            "summary": "Plan",
...
def test_codex_vault_runtime_blocks_event_shaped_mcp_invocation(tmp_path):
    handle = rail.start_task(_draft(_target_repo(tmp_path)))
    runner = FakeCodexRunner(
        final_output={
            "summary": "Plan",
        ...
        extra_events=[{"event": "mcp_invocation", "server": "filesystem"}],
    )
    runtime = _runtime(tmp_path, command=_fake_codex_command(tmp_path), runner=runner)
    ...
    assert result.status == "interrupted"
    assert result.blocked_category == "policy"
    assert result.structured_output["error"] == "MCP invocation is not allowed"
...
def test_codex_vault_runtime_blocks_tool_event_nested_in_content_array(tmp_path):
    ...
    runner = FakeCodexRunner(
        ...
        extra_events=[{"msg": {"type": "item.started", "content": [{"type": "mcp_tool_call", "server": "filesystem"}]}}],
    )
    runtime = _runtime(tmp_path, command=_fake_codex_command(tmp_path), runner=runner)
...
def test_codex_vault_runtime_blocks_tool_event_nested_in_content_object(tmp_path):
    handle = rail.start_task(_draft(_target_repo(tmp_path)))
    runner = FakeCodexRunner(
        final_output={
            "summary": "Plan",
...
def test_codex_vault_runtime_blocks_content_object_command_only_shell_event(tmp_path):
    handle = rail.start_task(_draft(_target_repo(tmp_path)))
    runner = FakeCodexRunner(
        final_output={
            "summary": "Plan",
        ...
        extra_events=[{"content": {"command": "touch app.txt"}}],
    )
    runtime = _runtime(tmp_path, command=_fake_codex_command(tmp_path), runner=runner)
...
def test_codex_vault_runtime_audits_all_nested_content_commands(tmp_path):
    handle = rail.start_task(_draft(_target_repo(tmp_path)))
    runner = FakeCodexRunner(
        final_output={
            "summary": "Plan",
        ...
        extra_events=[{"command": "pwd", "cwd": "__SANDBOX__", "content": [{"command": "touch app.txt"}]}],
    )
    runtime = _runtime(tmp_path, command=_fake_codex_command(tmp_path), runner=runner)
...
def test_codex_vault_runtime_preserves_list_shell_wrapper_script(tmp_path):
    handle = rail.start_task(_draft(_target_repo(tmp_path)))
    runner = FakeCodexRunner(
        final_output={
            "summary": "Plan",
        ...
        extra_events=[{"type": "command_execution", "cwd": "__SANDBOX__", "command": ["/bin/zsh", "-lc", "cat ../secret.txt"]}],
    )
    runtime = _runtime(tmp_path, command=_fake_codex_command(tmp_path), runner=runner)
...
def test_codex_vault_runtime_blocks_shell_brace_expansion_escape(tmp_path):
    handle = rail.start_task(_draft(_target_repo(tmp_path)))
    runner = FakeCodexRunner(
        final_output={
            "summary": "Plan",
        ...
        *,
        ...
        if command[1:3] == ["exec", "--json"]:
            ...
            if self.malformed_stdout:
                ...
                return codex_vault.CodexCommandRunResult(stdout=f"{stdout}\n{{not-json", stderr="", returncode=self.exec_returncode)
